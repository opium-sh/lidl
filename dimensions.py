...
class KNNComputerNoCheck(nn.Module):
    ...
    def __init__(self, sample_num, K=1, cosine_dist=False):
        ...
        if K == 1:
            ...
            self.register_buffer(
                "nn_indices", torch.full((sample_num,), 0, dtype=torch.int64)
            )
        else:
            self.register_buffer("min_dists", torch.full((sample_num, K), float("inf")))
            self.register_buffer(
                "nn_indices", torch.full((sample_num, K), 0, dtype=torch.int64)
            )
    ...
    def forward(self, x, x_idx_start, y, y_idx_start):
        ...
        if self.K == 1:
            ...
            self.nn_indices[x_idx_start : x_idx_start + x_bsize] = (
                nn_idxes + y_idx_start
            )
        else:
            comp = torch.cat(
                [dist, self.min_dists[x_idx_start : x_idx_start + x_bsize]], dim=1
            )
            ...
            for row in range(sorted_dists.shape[0]):
                ...
                while sidx < sorted_dists.shape[1]:
                    if sorted_dists[row, sidx] == 0:
                        ...
                        break
                updated_dist_list.append(
                    sorted_dists[row, sidx - 1 : sidx - 1 + self.K]
                )
...
def update_nn(
    anchor_loader, anchor_start_idx, new_img_loader, new_start_idx, nn_computer, device
):
    ...
    with torch.no_grad():
        for n, (abatch, _) in enumerate(anchor_loader):
            ...
            for newbatch, _ in new_img_loader:
                ...
                equiv_flag = (
                    nn_computer.min_dists[
                        anchor_start_idx : anchor_start_idx + abatch.size(0), 0
                    ]
                    == 0
                ) & (
                    nn_computer.min_dists[
                        anchor_start_idx : anchor_start_idx + abatch.size(0), 1
                    ]
                    == 0
                )
