    ...
    def run(self,
        ...
        for _ in range(num_layers):
            ...
            if self.flow_type == 'rqnsf':
                transforms.append(MaskedAffineAutoregressiveTransform(features=data.shape[1], 
                    hidden_features=5*data.shape[1]))
            elif self.flow_type == 'maf':
                transforms.append(MaskedPiecewiseRationalQuadraticAutoregressiveTransform(features=data.shape[1], 
                    hidden_features=15*data.shape[1], num_bins=5, num_blocks=5, 
                    tails="linear", tail_bound=4, use_batch_norm=False))
            else:
