...
class LLGaussianMixtures:
    ...
    def __call__(self, delta, dataset, test):
        ...
        tq1 = tqdm.tqdm(range(self.runs), leave=False, unit='run')
        for run in tq1:
            ...
            tq2 = tqdm.tqdm(n_comps, leave=False, unit='num_comp')
            for n_comp in tq2:
...
class LLFlow:
    ...
    def __call__(self, delta, dataset, test):
        ...
        losses = list()
        results = list()
        tq1 = tqdm.tqdm(range(self.epochs), leave=False)
        for epoch in tq1:
            tq1.set_description(f"epoch: {epoch + 1}")
            tq2 = tqdm.tqdm(DataLoader(train_tensor, batch_size=self.batch_size), leave=False)
            for x in tq2:
